...
RSpec.describe Product, type: :model do
  context 'validation' do
    ...
    it "ensure the presene of price" do
      product = Product.new(name: "name", description: 'description', price: 0, weight: 100).save
      expect(product).to eq (false)
    end

    it "ensure the presene of weight" do
      product = Product.new(name: "name", description: 'description', price: 100000, weight: 0).save
      expect(product).to eq (false)
    end
  end
end
